...
with col1:
    ...
    if len(st.session_state.cart) > 0:
        ...
        st.info("💡 您的购物车还是空的哦，请在上方选择肉类 and 数量并点击『放入我的购物车』！")
...
with col2:
    ...
    with st.container(border=True):
        ...
        2. **一键提交**：多选放入完成后，在下方核对您的菜品和总额，填写您**准确的微信昵称**，点击 **『确认提交拼单』**。当页面成功喷出小气球 🎈 并提示“提交成功”后，即可放心关闭网页。
        
        ---
        
        ### 📦 关于提货与核对
        * **1.** 所有的订单数据提交后，都会安全存入后台，数据不会因重复刷新而丢失。
        * **2.** 截单后，团长会把最终的对账总单打包群发回微信群。
        * **3.** 如有任何下单错误或修改需求，请直接在微信群里私信联系团长进行人工改动。
        """)
